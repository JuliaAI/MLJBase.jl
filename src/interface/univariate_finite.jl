...
function MMI.UnivariateFinite(::FI, c::AbstractVector{<:CategoricalElement},
                              p::AbstractVector{<:Real})
    # check that the vectors have appropriate length
    Dist.@check_args(UnivariateFinite, length(classes) == length(p))
    # construct a dict & feed it to the dictionary constructor
    pairs = (c[i] => p[i] for i in eachindex(p))
    return UnivariateFinite(LittleDict(pairs...))
end
